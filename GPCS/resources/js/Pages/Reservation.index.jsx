        ...
        <AuthenticatedLayout >
            <div>
                {reservations.data.length === 0 ? (
                    <div className="py-8">
                        <h1 className="text-2xl font-semibold mb-4">Récapitulatif de mes réservations</h1>
                            ...
                            <table className="w-full bg-white shadow-md rounded my-4">
                                ...
                                {reservations.data.map((reservation) => (
                                    <tr key={reservation.id} className="border-b border-gray-200">
                                        ...
                                        <td className="py-3 px-6 text-left">{reservation.price}€</td>
                                        <td className="py-3 px-6 text-left">{reservation.start_time}</td>
                                        <td className="py-3 px-6 text-left">{reservation.end_time}</td>
                                        <td className="py-3 px-6 text-left">{reservation.created_at}</td>
                                        <td className="py-3 px-6 text-left">{reservation.status}</td>
                                        <td className="py-3 px-6 text-center">
                                            {new Date().getTime() - new Date(reservation.start_time).getTime() < 48 * 60 * 60 * 1000 && reservation.status != "Refusé" &&(
                                                <button
                                                    className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                                                    onClick={(e) => handleCancelReservation(e, reservation.id)}
                                                >
                                                    Annuler
                                                </button>
                                            )}
