...
export default function Appartements({ appartement, auth, storagePath, fermetures, intervalles, reservedDates }) {
    useEffect(() => {
        const updateTotalPrice = () => {
            const startTime = new Date(document.getElementById('start_time').value);
            const endTime = new Date(document.getElementById('end_time').value);
            const numberOfPersons = parseInt(document.getElementById('nombre_de_personne').value, 10);
            ...
            if (!isNaN(startTime) && !isNaN(endTime) && startTime < endTime && numberOfPersons > 0 && !isNaN(pricePerNight)) {
                const numberOfNights = Math.ceil((endTime - startTime) / (1000 * 3600 * 24));
        ...
        const estDansIntervalle = (date, intervalles, fermetures) => {
            ...
            for (let i = 0; i < intervalles.length; i++) {
                ...
                const intervalleStartDate = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate());
                const intervalleEndDate = new Date(endDate.getFullYear(), endDate.getMonth(), endDate.getDate());
            ...
            for (let i = 0; i < fermetures.length; i++) {
                ...
                const trueFermetureStart = new Date(fermetureStart.getFullYear(), fermetureStart.getMonth(), fermetureStart.getDate());
                const trueFermetureEnd = new Date(fermetureEnd.getFullYear(), fermetureEnd.getMonth(), fermetureEnd.getDate());
                    ...
                    <article>
                        ...
                        <div className="flex justify-between mt-5">
                            ...
                            <div className="p-4 sm:p-8 ml-20 bg-white sm:rounded-lg shadow-xl">
                                <p className="text-xl">
                                    <span className="font-extrabold">{appartement.price}€</span> par nuit
                                </p>
                                ...
                                <form method="POST" action="/reservation/store">
                                    ...
                                    <div className="mb-4">
                                        <label htmlFor="nombre_de_personne" className="block text-gray-700 text-sm font-bold mb-2">
                                            Nombre de personnes :
                                        </label>
                                        <input
                                            type="number"
                                    ...
                                    <div className="mb-4" id="total_price_container" style={{ display: 'none' }}>
